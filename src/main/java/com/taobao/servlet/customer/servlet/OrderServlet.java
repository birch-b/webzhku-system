...
public class OrderServlet extends HttpServlet {
    ...
    private void showDetail(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try (Connection c = DBUtil.getConnection()) {
            PreparedStatement p = c.prepareStatement("SELECT o.*,s.shop_name,l.company,l.tracking_no,l.status AS logistics_status FROM `order` o LEFT JOIN shop s ON o.shop_id=s.id LEFT JOIN logistics l ON o.id=l.order_id WHERE o.id=? AND o.user_id=?");
            ...
            PreparedStatement prv = c.prepareStatement("SELECT id FROM review WHERE order_id=? AND user_id=?"); prv.setLong(1, oid); prv.setLong(2, uid); if (prv.executeQuery().next()) req.setAttribute("hasReview", true);
